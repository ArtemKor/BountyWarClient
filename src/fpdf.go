...
func (f *Fpdf) GetStringWidth(s string) float64 {
	...
	if f.unifontSubset {
		...
		for _, char := range unicode {
			if len(f.currentFont.Cw) >= int(char) && f.currentFont.Cw[int(char)] > 0 {
				w += f.currentFont.Cw[int(char)]
			} else if f.currentFont.Desc.MissingWidth != 0 {
				w += f.currentFont.Desc.MissingWidth
			} else if f.currentFont.MissingWidth != 0 {
				w += f.currentFont.MissingWidth
			} else {
				w += 500
			}
		}
...
func (f *Fpdf) addFont(familyStr, styleStr, fileStr string, uni bool) {
	if fileStr == "" {
		if uni {
	...
	if uni {
		ttfFileName := fileStr
		ttfStat, _ := os.Stat(ttfFileName)
		originalSize := ttfStat.Size()
		Type := "TTF"
		ttfFile := ttfFileName
		ttf := NewTTFontFile()
		err := ttf.GetMetrics(ttfFileName)
		if err != nil {
		...
		up := math.Round(ttf.UnderlinePosition)
...
func (f *Fpdf) SetFont(familyStr, styleStr string, size float64) {
	...
	var fontKey string
	if familyStr == "utf8" || familyStr == "dejavu" {
		familyStr = "utf8"
		fontKey = getFontKey(familyStr, styleStr)
		_, ok := f.fonts[fontKey]
		if !ok {
			...
			ttf := NewTTFontFile()
			if styleStr == "B"{
				cop := make([]byte,len(DejaVuBoldUTF8Font))
				copy(cop, DejaVuBoldUTF8Font[:])
				ttfFile = "utf8B"
				ttfArray = cop[:]
			} else if styleStr == "I"{
				cop := make([]byte,len(DejaVuItalicUTF8Font))
				copy(cop, DejaVuItalicUTF8Font[:])
				ttfFile = "utf8I"
				ttfArray = cop[:]
			} else if styleStr == "BI"{
				cop := make([]byte,len(DejaVuBoldItalicUTF8Font))
				copy(cop, DejaVuBoldItalicUTF8Font[:])
				ttfFile = "utf8BI"
				ttfArray = cop[:]
				//ttfArray = Asset("data/DejaVuSansCondensed-BoldOblique.ttf")
			} else {
				cop := make([]byte,len(DejaVuRegularUTF8Font))
				copy(cop, DejaVuRegularUTF8Font[:])
				ttfFile = "utf8"
				ttfArray = cop[:]
			}
			originalSize := int64(len(ttfArray))

			err := ttf.GetUTFMetrics(ttfArray)
			if err != nil {
				fmt.Printf("get metrics Error: %e\n", err)
				return
			}
			...
			f.fontFiles[ttfFile] = fontFileType{
				Type: "TTF",
			}
		}
	} else {
	...
	if f.currentFont.Tp == "TTF" {
		f.unifontSubset = true
	} else {
		f.unifontSubset = false
	}
	if f.page > 0 {
...
func (f *Fpdf) Text(x, y float64, txtStr string) {
	...
	if f.unifontSubset {
		...
		for _, uni := range []rune(txtStr) {
			f.currentFont.subset[int(uni)] = int(uni)
		}
	} else {
	...
	if len(txtStr) > 0 {
		...
		if f.ws != 0 && f.unifontSubset {
			...
			for i := 0; i < numt; i++ {
				...
				s.printf("%s ", tx)
				if (i + 1) < numt {
					adj := -(f.ws * f.k) * 1000 / f.fontSizePt
					s.printf("%d(%s) ", adj, space)
				}
			}
			s.printf("] TJ")
			s.printf(" ET")
...
func (f *Fpdf) MultiCell(w, h float64, txtStr, borderStr, alignStr string, fill bool) {
	...
	for i < nb {
		...
		if f.unifontSubset {
			//l += int(f.GetStringWidth(string(c)))
			l += int(float64(cw[int(c)]))// * f.fontSize / 1000.0)
		} else {
			//if len((*cw)) < int(c){
			//	fmt.Printf("Len %d \n", len(*cw))
			//}
...
func (f *Fpdf) write(h float64, txtStr string, link int, linkStr string) {
	...
	for i < nb {
		...
		if f.unifontSubset {
			//l += f.GetStringWidth(string(c))
			l += float64(cw[int(c)])
		} else {
			l += float64(cw[int(c)])// * f.fontSize / 1000.0
		}
		//l += float64(cw[c])
	...
	}
	{
		...
		for _, key = range keyList {
			...
			case "TrueType":
				...
				f.out("endobj")
			case "TTF":
				// Additional UTF-8 font
				ttf := NewTTFontFile()
				fontName := "MPDFAA" + "+" + font.Name
				subset := font.subset
				delete(subset, 0)
				var ttFontStream []byte
				if key == "utf8"{
					cop := make([]byte,len(DejaVuRegularUTF8Font))
					copy(cop, DejaVuRegularUTF8Font[:])
					ttFontStream = ttf.MakeArraySubset(cop[:], subset)
				}else if key == "utf8B"{
					cop := make([]byte,len(DejaVuBoldUTF8Font))
					copy(cop, DejaVuBoldUTF8Font[:])
					ttFontStream = ttf.MakeArraySubset(cop[:], subset)
				...
				compressedFontStream := sliceCompress(ttFontStream)
				...
				f.out("<</Type /Font")
				f.out("/Subtype /Type0")
				f.out("/BaseFont /" + fontName + "")
				f.out("/Encoding /Identity-H")
				f.out("/DescendantFonts [" + strconv.Itoa(f.n+1) + " 0 R]")
				f.out("/ToUnicode " + strconv.Itoa(f.n+2) + " 0 R")
				f.out(">>")
				f.out("endobj")
				...
				f.out("/BaseFont /" + fontName + "")
				f.out("/CIDSystemInfo " + strconv.Itoa(f.n+2) + " 0 R")
				f.out("/FontDescriptor " + strconv.Itoa(f.n+3) + " 0 R")
				if font.Desc.MissingWidth != 0 {
					f.out("/DW " + strconv.Itoa(font.Desc.MissingWidth) + "")
				}

				f._putTTfontwidths(&font, ttf.MaxUni)
				...
				toUni += "end\n"
				toUni += "end"
				f.out("<</Length " + strconv.Itoa(len(toUni)) + ">>")
				f.putstream([]byte(toUni))
				f.out("endobj")
				...
				// Font descriptor
				f.newobj()
				f.out("<</Type /FontDescriptor")
				f.out("/FontName /" + fontName)

				f.out(" /Ascent " + strconv.Itoa(font.Desc.Ascent))
				f.out(" /Descent " + strconv.Itoa(font.Desc.Descent))
				f.out(" /CapHeight " + strconv.Itoa(font.Desc.CapHeight))
				v := font.Desc.Flags
				v = v | 4
				v = v &^ 32
				f.out(" /Flags " + strconv.Itoa(v))
				f.out(" /FontBBox [" + strconv.Itoa(font.Desc.FontBBox.Xmin) + " " + strconv.Itoa(font.Desc.FontBBox.Ymin) + " " + strconv.Itoa(font.Desc.FontBBox.Xmax) + " " + strconv.Itoa(font.Desc.FontBBox.Ymax) + "]")
				f.out(" /ItalicAngle " + strconv.Itoa(font.Desc.ItalicAngle))
				f.out(" /StemV " + strconv.Itoa(font.Desc.StemV))
				f.out(" /MissingWidth " + strconv.Itoa(font.Desc.MissingWidth))

				f.out("/FontFile2 " + strconv.Itoa(f.n+2) + " 0 R")
				f.out(">>")
				f.out("endobj")
				...
				for cc, glyph := range codeToGlyph {
				...
				f.newobj()
				f.out("<</Length " + strconv.Itoa(len(cidToGidMap)) + "")
				f.out("/Filter /FlateDecode")
				f.out(">>")
				...
				f.out("/Filter /FlateDecode")
				f.out("/Length1 " + strconv.Itoa(ttFontSize))
				f.out(">>")
...
func (pa *phpArray) getIndex(key interface{}) int {
	if key != nil {
...
func (pa *phpArray) put(key interface{}, obj int) {
	if key == nil {
		i := 0
		for n := 0; ; n++ {
			i = pa.getIndex(n)
		...
		pa.objSet = append(pa.objSet, obj)
	} else {
		i := pa.getIndex(key)
		if i < 0 {
			pa.keySet = append(pa.keySet, key)
...
func (pa *phpArray) delete(key interface{}) {
	if pa == nil || pa.keySet == nil || pa.objSet == nil{
		return
	}
	i := pa.getIndex(key)
	if i >= 0 {
		if i == 0 {
			...
			pa.objSet = pa.objSet[1:]
		} else if i == len(pa.keySet)-1 {
			pa.keySet = pa.keySet[:len(pa.keySet)-1]
			pa.objSet = pa.objSet[:len(pa.objSet)-1]
		} else {
			pa.keySet = append(pa.keySet[:i], pa.keySet[i+1:]...)
			pa.objSet = append(pa.objSet[:i], pa.objSet[i+1:]...)
		}
	}
}

func (pa *phpArray) get(key interface{}) int {
	i := pa.getIndex(key)
	if i >= 0 {
		return pa.objSet[i]
	}
...
func (pa *phpArray) pop() {
	pa.keySet = pa.keySet[:len(pa.keySet)-1]
	pa.objSet = pa.objSet[:len(pa.objSet)-1]
}

func array_merge(arr1, arr2 *phpArray) *phpArray {
	answer := phpArray{}
	if arr1 == nil && arr2 == nil {
		...
		answer.objSet = arr1.objSet[:]
	} else if arr1 == nil {
		answer.keySet = arr2.keySet[:]
		answer.objSet = arr2.objSet[:]
	} else {
		answer.keySet = arr1.keySet[:]
		answer.objSet = arr1.objSet[:]
		for i := 0; i < len(arr2.keySet); i++ {
			if arr2.keySet[i] == "interval" {
				...
				if arr1.getIndex("interval") < 0 {
					answer.put("interval", arr2.objSet[i])
				} else {
...
func (f *Fpdf) _putTTfontwidths(font *fontDefType, maxUni int) {
	...
	for cid := startcid; cid < cwlen; cid++ {
		...
		//if (is_writable(dirname(f._getfontpath()."unifont/x"))) {
		...
		if numb, OK := font.subset[cid]; cid > 255 && (!OK || numb == 0) {
			continue
		}
		...
		if cid == prevcid+1 {
			if width == prevwidth {
				...
				if width == Range[rangeid].get(0) {
					Range[rangeid].put(nil, width)
				} else {
					...
					Range[rangeid] = &r
					RangeKeys = append(RangeKeys, rangeid)
					Range[rangeid].put(nil, prevwidth)
					Range[rangeid].put(nil, width)
				}
				interval = true
				Range[rangeid].put("interval", 1)
				ui:=0
				ui=ui+1
			} else {
				if interval {
	...
	nextk := -1
	prevint := false
	for g := 0; g < len(RangeKeys); {
		k := RangeKeys[g]
		ws := *Range[k]
		cws := len(ws.keySet)
		if (k == nextk) && (!prevint) && (ws.getIndex("interval") < 0 || cws < 4) {
			if Range[k].getIndex("interval") >= 0 {
				Range[k].delete("interval")
			}
			Range[prevk] = array_merge(Range[prevk], Range[k])
			//delete(Range, k)
			RangeKeys = remove(RangeKeys, k)
		} else {
			g++
			prevk = k
		}
	...
	for _, k := range RangeKeys {
		ws := Range[k]
		if len(array_count_values(ws.objSet)) == 1 {
			w.printf(" %d %d %d", k, k+len(ws.objSet)-1, ws.get(0))
		} else {
			w.printf(" %d [ %s ]\n", k, implode(" ", ws.objSet))
		}
	}
...
func array_count_values(mp []int) map[int]int {
	answer := make(map[int]int)
	for _, v := range mp {
		answer[v] = answer[v] + 1
	}
